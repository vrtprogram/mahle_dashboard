...
#     st.session_state.count_data = [0, 0, 0, 0, 0]
def fatch_incident_data(date):
    global Record_lost_time, First_aid, Near_mis, Fire_mtd
    ...
    new_dict=dict()
    new_dict['Recordable Lost Time']=0
    ...
    for i in rows:
        ...
        if date_from_db == f"{specific_date}":
            ...
            if category == "Fire MTD":
                ...
                Fire_mtd = incident
    count_data = [
        new_dict["Recordable Lost Time"],
        new_dict["First Aid"],
        new_dict["Recordable Accident"],
        new_dict["Near MIS"],
        new_dict["Fire"]
    ]
    # st.session_state.incident_data = incident_data
    # st.session_state.count_data = list(new_dict.values())
    sconn.close()
    return count_data

def incident_tracking():
    sconn = sqlite3.connect("database/safety.db")
    count_data = fatch_incident_data(on_date) 
    sconn.close()
    new_dict=dict()
    new_dict["Recordable Lost Time"] = count_data[0]
    new_dict["Recordable Accident"] = count_data[1]
    new_dict["First Aid"] = count_data[2]
    new_dict["Near MIS"] = count_data[3]
    new_dict["Fire"] = count_data[4]
...
#************ Unsafe_Incidences_Tracking Start ************#
uconn = sqlite3.connect("database/safety.db")
month = datetime.now().month
year = datetime.now().year
cur = uconn.cursor()
if month < 10:
    df = pd.read_sql_query(f"Select * From 'UNSAFE INCIDENCES' WHERE strftime('%Y-%m', date) = '{year}-0{month}' ", uconn)
else:
...
def unsafe_incident_tracking():
    ...
    with col_indices_A:
        st.markdown(f"""<div style='background-color:tomato;margin:1rem;padding-top:0.7rem;border:1px solid black;height:12rem;border-radius:0.7rem;font-weight:bold;box-shadow:5px 5px 10px;text-align:center'>Recordable Lost Time Injury FTD:
                <h4 style='color:white;font-weight:bold; padding-top:2rem;'>$$1</h4></div>""".replace("$$1",f"{RLT}"),unsafe_allow_html=True)
    with col_indices_B:
        st.markdown(f"""<div style='background-color:red;margin:1rem;padding-top:0.7rem;border:1px solid black;height:12rem;border-radius:0.7rem;font-weight:bold;box-shadow:5px 5px 10px;text-align:center'>Recordable Accident FTD:
                <h4 style='color:white;font-weight:bold; padding-top:2.3rem;'>$$2</h4></div>""".replace("$$2",f"{RAC}"),unsafe_allow_html=True)
    with col_indices_C:
        st.markdown(f"""<div style='background-color:orange;margin:1rem;padding-top:0.7rem;border:1px solid black;height:12rem;border-radius:0.7rem;font-weight:bold;box-shadow:5px 5px 10px;text-align:center'>First Aid FTD:
                <h4 style='color:white;font-weight:bold; padding-top:3.5rem;'>$$3</h4></div>""".replace("$$3",f"{FAD}"),unsafe_allow_html=True)
    with col_indices_D:
        st.markdown(f"""<div style='background-color:yellow;margin:1rem;padding-top:0.7rem;border:1px solid black;height:12rem;border-radius:0.7rem;font-weight:bold;box-shadow:5px 5px 10px;text-align:center'>Near MIS FTD:
                <h4 style='color:white;font-weight:bold; padding-top:3.5rem;'>$$4</h4></div>""".replace("$$4",f"{NMT}"),unsafe_allow_html=True)
    with col_indices_E:
        st.markdown(f"""<div style='background-color:skyblue;margin:1rem;padding-top:0.7rem;border:1px solid black;height:12rem;border-radius:0.7rem;font-weight:bold;box-shadow:5px 5px 10px;text-align:center'>Fire FTD:
                <h4 style='color:white;font-weight:bold; padding-top:3.5rem;'>$$5</h4></div>""".replace("$$5",f"{FMT}"),unsafe_allow_html=True)
    # st.markdown("---")
    ...
    with st.container():
        ...
        with col1:
            ...
            while current_date.month == month:
                ...
                df_week = pd.read_sql_query(
                    f"Select * from 'UNSAFE INCIDENCES' where date between '{start_of_week}' and '{end_of_week}'", uconn)
        ...
        with col2:
            ...
            for i in range(0, 12):
                if i < 10:
                    df_month = pd.read_sql_query(f"Select * From 'UNSAFE INCIDENCES' WHERE strftime('%m', date) = '0{i + 1}' ", uconn)
                else:
                    df_month = pd.read_sql_query(f"Select * From 'UNSAFE INCIDENCES' WHERE strftime('%m', date) = '{i + 1}' ", uconn)
    ...
    df_new = pd.read_sql_query("""SELECT * FROM 'UNSAFE INCIDENCES' WHERE STATUS='Open' ORDER BY DATE DESC """, uconn)
    # df1 = pd.read_sql_query("""SELECT * FROM 'UNSAFE INCIDENCES'""",uconn)   
    # print(df_new)
    st.table(df_new[["DATE", "CATEGORY", "EVENT", "ACTION", "LOCATION", "STATUS"]])
#************ Unsafe_Incidences_Tracking End ************#

#************ Unsafe_Practice_Tracking Start ************#
pconn = sqlite3.connect("database/safety.db")
month = datetime.now().month
year = datetime.now().year
cur = pconn.cursor()
if month < 10:
    df = pd.read_sql_query(f"Select * From 'UNSAFE PRACTICES TRACKING' WHERE strftime('%Y-%m', date) = '{year}-0{month}' ", pconn)
else:
    df = pd.read_sql_query(
...
def unsafe_practice_tracking():
    ...
    with st.container():
        ...
        with col1:
            ...
            while current_date.month == month:
                ...
                current_date += timedelta(days=7)
                df_week = pd.read_sql_query(
                    f"Select * from 'UNSAFE PRACTICES TRACKING' where date between '{start_of_week}' and '{end_of_week}'", pconn)
        ...
        with col2:
            ...
            for i in range(0, 12):
                if i < 10:
                    df_month = pd.read_sql_query(f"Select * From 'UNSAFE PRACTICES TRACKING' WHERE strftime('%m', date) = '0{i + 1}' ", pconn)
                else:
                    df_month = pd.read_sql_query(f"Select * From 'UNSAFE PRACTICES TRACKING' WHERE strftime('%m', date) = '{i + 1}' ", pconn)
                lst.append(len(df_month))
    ...
    st.markdown("___")
    st.markdown("## Open Observations :")
    df_new = pd.read_sql_query("""SELECT * FROM 'UNSAFE PRACTICES TRACKING' WHERE STATUS='Open' ORDER BY DATE DESC """, pconn)
    # print(df_new)
    st.table(df_new[["DATE", "EVENT", "LOCATION", "STATUS"]])

#************ Unsafe_Practice_Tracking End ************#
    ...
    with col2:
        ...
        with blk2:
            ...
            <style>
                    ...
                    .par {padding-top:0rem; line-height:0.1rem; font-size:0.7rem; color:black; }
                    hr{ margin:0em; }
            </style>
            <div class="float-container">
                <div class="float-bd1">Top 3 Productivity Problems<hr>
            ...
            <div class="float-container">
                <div class="float-bd1">Top 3 OEE Related Problems<hr>
